...
DataWriter::DataWriter(DataWriterImpl* impl, const StatusMask& mask)
    : DomainEntity(mask), impl_(impl) {}

DataWriter::DataWriter(Publisher* pub, Topic* topic, const DataWriterQos& qos,
                       DataWriterListener* listener, const StatusMask& mask)
    : DomainEntity(mask),
...
ReturnCode_t DataWriter::loan_sample(void*& sample,
                                     LoanInitializationKind initialization) {
  return impl_->loan_sample(sample, initialization);
}

ReturnCode_t DataWriter::discard_loan(void*& sample) {
  return impl_->discard_loan(sample);
}

bool DataWriter::write(void* data) { return impl_->write(data); }

bool DataWriter::write(void* data, fastrtps::rtps::WriteParams& params) {
  return impl_->write(data, params);
}

ReturnCode_t DataWriter::write(void* data, const InstanceHandle_t& handle) {
  return impl_->write(data, handle);
}

ReturnCode_t DataWriter::write_w_timestamp(void* data,
...
InstanceHandle_t DataWriter::register_instance(void* instance) {
  return impl_->register_instance(instance);
}

InstanceHandle_t DataWriter::register_instance_w_timestamp(
    void* instance, const fastrtps::Time_t& timestamp) {
  return impl_->register_instance_w_timestamp(instance, timestamp);
}

InstanceHandle_t DataWriter::register_instance_w_timestamp(
    void* instance, const fastrtps::rtps::Time_t& timestamp) {
  static_cast<void>(instance);
  static_cast<void>(timestamp);
  EPROSIMA_LOG_WARNING(
      DATA_WRITER, "register_instance_w_timestamp method not yet implemented");
  return HANDLE_NIL;
}

ReturnCode_t DataWriter::unregister_instance(void* instance,
                                             const InstanceHandle_t& handle) {
...
ReturnCode_t DataWriter::get_key_value(void* key_holder,
                                       const InstanceHandle_t& handle) {
  return impl_->get_key_value(key_holder, handle);
}

InstanceHandle_t DataWriter::lookup_instance(const void* instance) const {
  static_cast<void>(instance);
  EPROSIMA_LOG_WARNING(DATA_WRITER, "lookup_instance method not implemented");
  return HANDLE_NIL;
}

ReturnCode_t DataWriter::dispose(void* data, const InstanceHandle_t& handle) {
...
const fastrtps::rtps::GUID_t& DataWriter::guid() const { return impl_->guid(); }

InstanceHandle_t DataWriter::get_instance_handle() const {
  return impl_->get_instance_handle();
}

TypeSupport DataWriter::get_type() const { return impl_->get_type(); }

ReturnCode_t DataWriter::set_qos(const DataWriterQos& qos) {
  return impl_->set_qos(qos);
}

const DataWriterQos& DataWriter::get_qos() const { return impl_->get_qos(); }

ReturnCode_t DataWriter::get_qos(DataWriterQos& qos) const {
  qos = impl_->get_qos();
  return ReturnCode_t::RETCODE_OK;
}
...
Topic* DataWriter::get_topic() const { return impl_->get_topic(); }
...
ReturnCode_t DataWriter::get_matched_subscription_data(
  ...
  fprintf(fp, "DataWriter::get_matched_subscription_data\t%d\n",
...
ReturnCode_t DataWriter::get_matched_subscriptions(
  ...
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriter::get_matched_subscriptions\t%d\n",
          ReturnCode_t::RETCODE_UNSUPPORTED);
  fclose(fp);
  return ReturnCode_t::RETCODE_UNSUPPORTED;
  /*
