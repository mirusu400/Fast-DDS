...
static ChangeKind_t unregister_change_kind(bool dispose,
...
static bool qos_has_pull_mode_request(const DataWriterQos& qos) {
  auto push_mode = PropertyPolicyHelper::find_property(qos.properties(),
                                                       "fastdds.push_mode");
...
class DataWriterImpl::LoanCollection {
 public:
  explicit LoanCollection(const PoolConfig& config)
      : loans_(get_collection_limits(config)) {}

  bool add_loan(void* data, PayloadInfo_t& payload) {
    static_cast<void>(data);
    assert(data == payload.payload.data +
                       SerializedPayload_t::representation_header_size);
    return loans_.push_back(payload);
  }

  bool check_and_remove_loan(void* data, PayloadInfo_t& payload) {
    octet* payload_data = static_cast<octet*>(data) -
                          SerializedPayload_t::representation_header_size;
 ...
 private:
  static ResourceLimitedContainerConfig get_collection_limits(
    ...
    return {config.initial_size, config.maximum_size,
...
DataWriterImpl::DataWriterImpl(PublisherImpl* p, TypeSupport type, Topic* topic,
    ...
    : publisher_(p),
      type_(type),
      topic_(topic),
      qos_(&qos == &DATAWRITER_QOS_DEFAULT
               ? publisher_->get_default_datawriter_qos()
               : qos),
      listener_(listen),
      history_(get_topic_attributes(qos_, *topic_, type_), type_->m_typeSize,
               ...
               [this](const InstanceHandle_t& handle) -> void {
                 if (nullptr != listener_) {
                   listener_->on_unacknowledged_sample_removed(user_datawriter_,
                                                               handle);
                 }
               })
#pragma warning(disable : 4355)
  ...
  fastrtps::rtps::RTPSParticipantImpl::preprocess_endpoint_attributes<
      WRITER, 0x03, 0x02>(EntityId_t::unknown(),
                          publisher_->get_participant_impl()->id_counter(),
                          endpoint_attributes, guid_.entityId);
...
ReturnCode_t DataWriterImpl::enable() {
  ...
  w_att.endpoint.reliabilityKind =
      qos_.reliability().kind == RELIABLE_RELIABILITY_QOS ? RELIABLE
                                                          : BEST_EFFORT;
  ...
  w_att.endpoint.external_unicast_locators =
      qos_.endpoint().external_unicast_locators;
  w_att.endpoint.ignore_non_matching_locators =
      qos_.endpoint().ignore_non_matching_locators;
  ...
  w_att.matched_readers_allocation =
      qos_.writer_resource_limits().matched_subscriber_allocation;
  w_att.disable_heartbeat_piggyback =
      qos_.reliable_writer_qos().disable_heartbeat_piggyback;
  ...
  std::string* endpoint_partitions =
      PropertyPolicyHelper::find_property(qos_.properties(), "partitions");
  ...
  if (qos_.reliable_writer_qos().disable_positive_acks.enabled &&
      qos_.reliable_writer_qos().disable_positive_acks.duration !=
          c_TimeInfinite) {
  ...
  ReturnCode_t ret_code =
      check_datasharing_compatible(w_att, is_data_sharing_compatible_);
  ...
  if (filtering_enabled) {
    reader_filters_.reset(new ReaderFilterCollection(
        qos_.writer_resource_limits().reader_filters_allocation));
  }

  auto change_pool = get_change_pool();
  if (!change_pool) {
    EPROSIMA_LOG_ERROR(DATA_WRITER,
                       "Problem creating change pool for associated Writer");
  ...
  if (!pool) {
    EPROSIMA_LOG_ERROR(DATA_WRITER,
                       "Problem creating payload pool for associated Writer");
    return ReturnCode_t::RETCODE_ERROR;
  }

  RTPSWriter* writer = RTPSDomainImpl::create_rtps_writer(
      publisher_->rtps_participant(), guid_.entityId, w_att, pool, change_pool,
      static_cast<WriterHistory*>(&history_),
      static_cast<WriterListener*>(&writer_listener_));

  if (writer == nullptr && w_att.endpoint.data_sharing_configuration().kind() ==
                               DataSharingKind::AUTO) {
    ...
    writer = RTPSDomainImpl::create_rtps_writer(
        publisher_->rtps_participant(), guid_.entityId, w_att, pool,
        change_pool, static_cast<WriterHistory*>(&history_),
        static_cast<WriterListener*>(&writer_listener_));
  }
  if (writer == nullptr) {
    release_payload_pool();
  ...
  // In case it has been loaded from the persistence DB, rebuild instances on
  // history
  history_.rebuild_instances();

  deadline_timer_ = new TimedEvent(
      publisher_->get_participant()->get_resource_event(),
      [&]() -> bool { return deadline_missed(); },
      qos_.deadline().period.to_ns() * 1e-6);

  lifespan_timer_ = new TimedEvent(
      publisher_->get_participant()->get_resource_event(),
      [&]() -> bool { return lifespan_expired(); },
  ...
  // REGISTER THE WRITER
  WriterQos wqos =
      qos_.get_writerqos(get_publisher()->get_qos(), topic_->get_qos());
  if (!is_data_sharing_compatible_) {
  ...
  publisher_->rtps_participant()->registerWriter(
      writer_, get_topic_attributes(qos_, *topic_, type_), wqos);
...
DataWriterImpl::~DataWriterImpl() {
  ...
  if (writer_ != nullptr) {
    EPROSIMA_LOG_INFO(DATA_WRITER,
                      guid().entityId << " in topic: " << type_->getName());
...
ReturnCode_t DataWriterImpl::loan_sample(
    void*& sample, LoanInitializationKind initialization) {
  // Type should be plain and have space for the representation header
  if (!type_->is_plain() ||
      SerializedPayload_t::representation_header_size > type_->m_typeSize) {
  ...
  if (!get_free_payload_from_pool([size]() { return size; }, payload)) {
  ...
  sample =
      payload.payload.data + SerializedPayload_t::representation_header_size;
  ...
  switch (initialization) {
    default:
      EPROSIMA_LOG_WARNING(DATA_WRITER,
                           "Using wrong LoanInitializationKind value ("
...
ReturnCode_t DataWriterImpl::discard_loan(void*& sample) {
  // Type should be plain and have space for the representation header
  if (!type_->is_plain() ||
      SerializedPayload_t::representation_header_size > type_->m_typeSize) {
    return ReturnCode_t::RETCODE_ILLEGAL_OPERATION;
...
bool DataWriterImpl::write(void* data) {
  ...
  bool returned_value =
      ReturnCode_t::RETCODE_OK == create_new_change(ALIVE, data);
...
bool DataWriterImpl::write(void* data, fastrtps::rtps::WriteParams& params) {
  if (writer_ == nullptr) {
    return false;
  }

  EPROSIMA_LOG_INFO(DATA_WRITER, "Writing new data with WriteParams");
  bool returned_value = ReturnCode_t::RETCODE_OK ==
                        create_new_change_with_params(ALIVE, data, params);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::write\t%d\n", returned_value);
  fclose(fp);
  return returned_value;
...
ReturnCode_t DataWriterImpl::check_write_preconditions(
    void* data, const InstanceHandle_t& handle,
...
#if HAVE_SECURITY
    is_key_protected =
        writer_->getAttributes().security_attributes().is_key_protected;
...
ReturnCode_t DataWriterImpl::write_w_timestamp(
  ...
  if (ReturnCode_t::RETCODE_OK == ret) {
    EPROSIMA_LOG_INFO(DATA_WRITER,
                      "Writing new data with Handle and timestamp");
    WriteParams wparams;
...
InstanceHandle_t DataWriterImpl::register_instance(void* key) {
  ...
  if (ReturnCode_t::RETCODE_OK !=
      check_instance_preconditions(key, HANDLE_NIL, instance_handle)) {
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "DataWriterImpl::register_instance\t%p\n", HANDLE_NIL);
    fclose(fp);
    return HANDLE_NIL;
  }

  WriteParams wparams;
  InstanceHandle_t ret_val =
      do_register_instance(key, instance_handle, wparams);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::register_instance\t%p\n", ret_val);
  fclose(fp);
  return ret_val;
...
InstanceHandle_t DataWriterImpl::register_instance_w_timestamp(
    void* key, const fastrtps::Time_t& timestamp) {
  /// Preconditions
  InstanceHandle_t instance_handle;
  if (timestamp.is_infinite() || timestamp.seconds < 0 ||
      (ReturnCode_t::RETCODE_OK !=
       check_instance_preconditions(key, HANDLE_NIL, instance_handle))) {
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "DataWriterImpl::register_instance_w_timestamp\t%p\n",
            HANDLE_NIL);
    fclose(fp);
    return HANDLE_NIL;
  }
...
InstanceHandle_t DataWriterImpl::do_register_instance(
    void* key, const InstanceHandle_t instance_handle, WriteParams& wparams) {
  // TODO(MiguelCompany): wparams should be used when propagating the
  // register_instance operation to the DataReader. See redmine issue #14494
  static_cast<void>(wparams);

  // Block lowlevel writer
  auto max_blocking_time =
      std::chrono::steady_clock::now() +
      std::chrono::microseconds(::TimeConv::Time_t2MicroSecondsInt64(
    ...
    if (history_.register_instance(instance_handle, lock, max_blocking_time,
      ...
      if (0 == payload->length || nullptr == payload->data) {
        uint32_t size = fixed_payload_size_
                            ? fixed_payload_size_
                            : type_->getSerializedSizeProvider(key)();
        payload->reserve(size);
        if (!type_->serialize(key, payload)) {
          EPROSIMA_LOG_WARNING(DATA_WRITER, "Key data serialization failed");

          // Serialization of the sample failed. Remove the instance to keep
          // original state. Note that we will only end-up here if the instance
          // has just been created, so it will be empty and removing its changes
          // will remove the instance completely.
...
ReturnCode_t DataWriterImpl::unregister_instance(void* instance,
                                                 const InstanceHandle_t& handle,
                                                 bool dispose) {
  // Preconditions
  InstanceHandle_t ih;
  ReturnCode_t returned_value =
      check_instance_preconditions(instance, handle, ih);
  if (ReturnCode_t::RETCODE_OK == returned_value &&
      !history_.is_key_registered(ih)) {
  ...
  if (ReturnCode_t::RETCODE_OK == returned_value) {
    ...
    returned_value =
        create_new_change_with_params(change_kind, instance, wparams, ih);
  }
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::unregister_instance\t%d\n", returned_value);
  fclose(fp);
...
ReturnCode_t DataWriterImpl::unregister_instance_w_timestamp(
    void* instance, const InstanceHandle_t& handle,
    const fastrtps::Time_t& timestamp, bool dispose) {
  // Preconditions
  InstanceHandle_t instance_handle;
  ReturnCode_t ret = ReturnCode_t::RETCODE_OK;
  if (timestamp.is_infinite() || timestamp.seconds < 0) {
  ...
  if (ReturnCode_t::RETCODE_OK == ret &&
      !history_.is_key_registered(instance_handle)) {
  ...
  if (ReturnCode_t::RETCODE_OK == ret) {
    ...
    wparams.source_timestamp(timestamp);
    ChangeKind_t change_kind = unregister_change_kind(dispose, qos_);
    ret = create_new_change_with_params(change_kind, instance, wparams,
                                        instance_handle);
  }
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
ReturnCode_t DataWriterImpl::get_key_value(void* key_holder,
                                           const InstanceHandle_t& handle) {
  /// Preconditions
...
ReturnCode_t DataWriterImpl::create_new_change(ChangeKind_t changeKind,
                                               void* data) {
  WriteParams wparams;
...
ReturnCode_t DataWriterImpl::check_new_change_preconditions(
    ChangeKind_t change_kind, void* data) {
  // Preconditions
  if (data == nullptr) {
    EPROSIMA_LOG_ERROR(DATA_WRITER, "Data pointer not valid");
    return ReturnCode_t::RETCODE_BAD_PARAMETER;
  ...
  if (change_kind == NOT_ALIVE_UNREGISTERED ||
    ...
    if (!type_->m_isGetKeyDefined) {
      EPROSIMA_LOG_ERROR(DATA_WRITER,
                         "Topic is NO_KEY, operation not permitted");
      return ReturnCode_t::RETCODE_ILLEGAL_OPERATION;
    }
  }
...
ReturnCode_t DataWriterImpl::perform_create_new_change(
    ChangeKind_t change_kind, void* data, WriteParams& wparams,
    const InstanceHandle_t& handle) {
  // Block lowlevel writer
  auto max_blocking_time =
      steady_clock::now() + microseconds(::TimeConv::Time_t2MicroSecondsInt64(
                                qos_.reliability().max_blocking_time));
  ...
  if (!was_loaned) {
    if (!get_free_payload_from_pool(type_->getSerializedSizeProvider(data),
                                    payload)) {
      return ReturnCode_t::RETCODE_OUT_OF_RESOURCES;
    }
  ...
  if (ch != nullptr) {
    ...
    if (reader_filters_) {
      ...
      auto filter_hook = [&related_sample_identity, this](CacheChange_t& ch) {
        reader_filters_->update_filter_info(
            static_cast<DataWriterFilteredChange&>(ch),
            related_sample_identity);
      };
      added = history_.add_pub_change_with_commit_hook(ch, wparams, filter_hook,
                                                       lock, max_blocking_time);
    } else {
    ...
    if (qos_.deadline().period != c_TimeInfinite) {
      if (!history_.set_next_deadline(
              handle,
              steady_clock::now() + duration_cast<system_clock::duration>(
                                        deadline_duration_us_))) {
        EPROSIMA_LOG_ERROR(DATA_WRITER,
                           "Could not set the next deadline in the history");
      } else {
    ...
    if (qos_.lifespan().duration != c_TimeInfinite) {
      ...
      lifespan_timer_->update_interval_millisec(
          qos_.lifespan().duration.to_ns() * 1e-6);
      lifespan_timer_->restart_timer();
    }
...
ReturnCode_t DataWriterImpl::create_new_change_with_params(
    ChangeKind_t changeKind, void* data, WriteParams& wparams) {
...
ReturnCode_t DataWriterImpl::clear_history(size_t* removed) {
  return (history_.removeAllChange(removed) ? ReturnCode_t::RETCODE_OK
                                            : ReturnCode_t::RETCODE_ERROR);
}
...
const GUID_t& DataWriterImpl::guid() const { return guid_; }

InstanceHandle_t DataWriterImpl::get_instance_handle() const { return guid(); }
...
void DataWriterImpl::publisher_qos_updated() {
  if (writer_ != nullptr) {
    ...
    WriterQos wqos =
        qos_.get_writerqos(get_publisher()->get_qos(), topic_->get_qos());
    publisher_->rtps_participant()->updateWriter(
        writer_, get_topic_attributes(qos_, *topic_, type_), wqos);
  }
}
...
ReturnCode_t DataWriterImpl::set_qos(const DataWriterQos& qos) {
  ...
  const DataWriterQos& qos_to_set =
  ...
  if (&qos != &DATAWRITER_QOS_DEFAULT) {
    ReturnCode_t ret_val =
        check_qos_including_resource_limits(qos_to_set, type_);
    if (!ret_val) {
      return ret_val;
    }

    if (publisher_->get_participant()
                ->get_qos()
                .allocation()
                .data_limits.max_user_data != 0 &&
  ...
  if (enabled) {
    ...
    // Deadline
    if (qos_.deadline().period != c_TimeInfinite) {
      deadline_duration_us_ = duration<double, std::ratio<1, 1000000>>(
          qos_.deadline().period.to_ns() * 1e-3);
      deadline_timer_->update_interval_millisec(qos_.deadline().period.to_ns() *
                                                1e-6);
    } else {
      deadline_timer_->cancel_timer();
    }
...
const DataWriterQos& DataWriterImpl::get_qos() const { return qos_; }

ReturnCode_t DataWriterImpl::set_listener(DataWriterListener* listener) {
  listener_ = listener;
  return ReturnCode_t::RETCODE_OK;
}
...
const Publisher* DataWriterImpl::get_publisher() const {
  const Publisher* ret_val = publisher_->get_publisher();
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
void DataWriterImpl::InnerDataWriterListener::onWriterMatched(
    RTPSWriter* /*writer*/, const PublicationMatchedStatus& info) {
  ...
  if (listener != nullptr) {
    ...
    if (ReturnCode_t::RETCODE_OK ==
        data_writer_->get_publication_matched_status(callback_status)) {
      listener->on_publication_matched(data_writer_->user_datawriter_,
                                       callback_status);
    }
  }
  data_writer_->user_datawriter_->get_statuscondition().get_impl()->set_status(
      notify_status, true);
}

void DataWriterImpl::InnerDataWriterListener::on_offered_incompatible_qos(
    RTPSWriter* /*writer*/, fastdds::dds::PolicyMask qos) {
  ...
  if (listener != nullptr) {
    ...
    if (data_writer_->get_offered_incompatible_qos_status(callback_status) ==
        ReturnCode_t::RETCODE_OK) {
      listener->on_offered_incompatible_qos(data_writer_->user_datawriter_,
                                            callback_status);
    }
  }
  data_writer_->user_datawriter_->get_statuscondition().get_impl()->set_status(
...
void DataWriterImpl::InnerDataWriterListener::onWriterChangeReceivedByAll(
    RTPSWriter* /*writer*/, CacheChange_t* ch) {
...
void DataWriterImpl::InnerDataWriterListener::on_liveliness_lost(
  ...
  if (listener != nullptr) {
    ...
    if (ReturnCode_t::RETCODE_OK ==
        data_writer_->get_liveliness_lost_status(callback_status)) {
      listener->on_liveliness_lost(data_writer_->user_datawriter_,
                                   callback_status);
    }
  }
  data_writer_->user_datawriter_->get_statuscondition().get_impl()->set_status(
...
void DataWriterImpl::InnerDataWriterListener::on_reader_discovery(
  ...
  if (!fastrtps::rtps::RTPSDomainImpl::should_intraprocess_between(
          writer->getGuid(), reader_guid)) {
    switch (reason) {
...
ReturnCode_t DataWriterImpl::wait_for_acknowledgments(
  ...
  if (history_.wait_for_acknowledgement_last_change(ih, lock,
                                                    max_blocking_time)) {
    return ReturnCode_t::RETCODE_OK;
  }
...
void DataWriterImpl::update_publication_matched_status(
  ...
  publication_matched_status_.last_subscription_handle =
...
ReturnCode_t DataWriterImpl::get_publication_matched_status(
  ...
  user_datawriter_->get_statuscondition().get_impl()->set_status(
      StatusMask::publication_matched(), false);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::get_publication_matched_status\t%d\n",
          ReturnCode_t::RETCODE_OK);
  fclose(fp);
...
bool DataWriterImpl::deadline_timer_reschedule() {
  ...
  if (!history_.get_next_deadline(timer_owner_, next_deadline_us)) {
    EPROSIMA_LOG_ERROR(DATA_WRITER,
                       "Could not get the next deadline from the history");
    return false;
  }

  auto interval_ms =
      duration_cast<milliseconds>(next_deadline_us - steady_clock::now());
  deadline_timer_->update_interval_millisec(
      static_cast<double>(interval_ms.count()));
  return true;
}
...
bool DataWriterImpl::deadline_missed() {
  ...
  if (nullptr != listener) {
    listener->on_offered_deadline_missed(user_datawriter_,
                                         deadline_missed_status_);
    deadline_missed_status_.total_count_change = 0;
  }
  user_datawriter_->get_statuscondition().get_impl()->set_status(notify_status,
                                                                 true);

  if (!history_.set_next_deadline(
          timer_owner_,
          steady_clock::now() +
              duration_cast<system_clock::duration>(deadline_duration_us_))) {
    EPROSIMA_LOG_ERROR(DATA_WRITER,
                       "Could not set the next deadline in the history");
    return false;
  }
...
ReturnCode_t DataWriterImpl::get_offered_deadline_missed_status(
  ...
  user_datawriter_->get_statuscondition().get_impl()->set_status(
      StatusMask::offered_deadline_missed(), false);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::get_offered_deadline_missed_status\t%d\n",
          ReturnCode_t::RETCODE_OK);
  fclose(fp);
...
ReturnCode_t DataWriterImpl::get_offered_incompatible_qos_status(
  ...
  user_datawriter_->get_statuscondition().get_impl()->set_status(
      StatusMask::offered_incompatible_qos(), false);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::get_offered_incompatible_qos_status\t%d\n",
          ReturnCode_t::RETCODE_OK);
  fclose(fp);
...
bool DataWriterImpl::lifespan_expired() {
  ...
  while (history_.get_earliest_change(&earliest_change)) {
    auto source_timestamp =
        ...
        nanoseconds(earliest_change->sourceTimestamp.to_ns());
    auto now = system_clock::now();

    // Check that the earliest change has expired (the change which started the
    // timer could have been removed from the history)
    if (now - source_timestamp < lifespan_duration_us_) {
      auto interval = source_timestamp - now + lifespan_duration_us_;
      lifespan_timer_->update_interval_millisec(
          static_cast<double>(duration_cast<milliseconds>(interval).count()));
      return true;
    }
    ...
    source_timestamp = system_clock::time_point() +
                       nanoseconds(earliest_change->sourceTimestamp.to_ns());
...
ReturnCode_t DataWriterImpl::get_liveliness_lost_status(
  ...
  user_datawriter_->get_statuscondition().get_impl()->set_status(
      StatusMask::liveliness_lost(), false);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DataWriterImpl::get_liveliness_lost_status\t%d\n",
          ReturnCode_t::RETCODE_OK);
  fclose(fp);
...
ReturnCode_t DataWriterImpl::assert_liveliness() {
  ...
  if (!publisher_->rtps_participant()->wlp()->assert_liveliness(
          writer_->getGuid(), writer_->get_liveliness_kind(),
          writer_->get_liveliness_lease_duration())) {
    EPROSIMA_LOG_ERROR(DATAWRITER, "Could not assert liveliness of writer "
                                       << writer_->getGuid());
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "DataWriterImpl::assert_liveliness\t%d\n",
            ReturnCode_t::RETCODE_ERROR);
  ...
  if (qos_.liveliness().kind == MANUAL_BY_TOPIC_LIVELINESS_QOS) {
    // As described in the RTPS specification, if liveliness kind is manual a
    // heartbeat must be sent This only applies to stateful writers, as
    // stateless writers do not send heartbeats
...
fastrtps::TopicAttributes DataWriterImpl::get_topic_attributes(
    const DataWriterQos& qos, const Topic& topic, const TypeSupport& type) {
  fastrtps::TopicAttributes topic_att;
  ...
  for (uint32_t id = 1; id < NEXT_QOS_POLICY_ID; ++id) {
    if (incompatible_policies.test(id)) {
      ++offered_incompatible_qos_status_
            .policies[static_cast<QosPolicyId_t>(id)]
            .count;
      offered_incompatible_qos_status_.last_policy_id =
          static_cast<QosPolicyId_t>(id);
    }
  }
...
LivelinessLostStatus& DataWriterImpl::update_liveliness_lost_status(
  ...
  liveliness_lost_status_.total_count_change +=
      liveliness_lost_status.total_count_change;
  return liveliness_lost_status_;
}

void DataWriterImpl::set_qos(DataWriterQos& to, const DataWriterQos& from,
                             bool is_default) {
...
ReturnCode_t DataWriterImpl::check_qos_including_resource_limits(
    const DataWriterQos& qos, const TypeSupport& type) {
...
ReturnCode_t DataWriterImpl::check_qos(const DataWriterQos& qos) {
  ...
  if (is_pull_mode) {
    if (BEST_EFFORT_RELIABILITY_QOS == qos.reliability().kind) {
      EPROSIMA_LOG_ERROR(RTPS_QOS_CHECK,
                         "BEST_EFFORT incompatible with pull mode");
      return ReturnCode_t::RETCODE_INCONSISTENT_POLICY;
    }
    if (c_TimeInfinite == qos.reliable_writer_qos().times.heartbeatPeriod) {
      EPROSIMA_LOG_ERROR(
          RTPS_QOS_CHECK,
          "Infinite heartbeat period incompatible with pull mode");
      return ReturnCode_t::RETCODE_INCONSISTENT_POLICY;
    }
  }
  if (qos.liveliness().kind == AUTOMATIC_LIVELINESS_QOS ||
    ...
    if (qos.liveliness().lease_duration < eprosima::fastrtps::c_TimeInfinite &&
        qos.liveliness().lease_duration <=
            qos.liveliness().announcement_period) {
      EPROSIMA_LOG_ERROR(RTPS_QOS_CHECK,
                         "WRITERQOS: LeaseDuration <= announcement period.");
      return ReturnCode_t::RETCODE_INCONSISTENT_POLICY;
    }
  }
  if (qos.data_sharing().kind() == DataSharingKind::ON &&
    ...
    EPROSIMA_LOG_ERROR(RTPS_QOS_CHECK,
                       "DATA_SHARING cannot be used with memory policies other "
                       "than PREALLOCATED.");
    return ReturnCode_t::RETCODE_INCONSISTENT_POLICY;
  }
  return ReturnCode_t::RETCODE_OK;
...
ReturnCode_t DataWriterImpl::check_allocation_consistency(
  ...
  if ((qos.resource_limits().max_samples > 0) &&
    ...
    EPROSIMA_LOG_ERROR(DDS_QOS_CHECK,
                       "max_samples should be greater than max_instances * "
                       "max_samples_per_instance");
    return ReturnCode_t::RETCODE_INCONSISTENT_POLICY;
  }
  if ((qos.resource_limits().max_instances <= 0 ||
...
bool DataWriterImpl::can_qos_be_updated(const DataWriterQos& to,
                                        const DataWriterQos& from) {
  bool updatable = true;
  if (to.durability().kind != from.durability().kind) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Durability kind cannot be changed after the creation "
                         "of a DataWriter.");
  }
  ...
  if (to.liveliness().kind != from.liveliness().kind) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Liveliness Kind cannot be changed after the creation "
                         "of a DataWriter.");
  }
  ...
  if (to.liveliness().lease_duration != from.liveliness().lease_duration) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Liveliness lease duration cannot be changed after "
                         "the creation of a DataWriter.");
  }

  if (to.liveliness().announcement_period !=
      from.liveliness().announcement_period) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Liveliness announcement cannot be changed after the "
  ...
  if (to.reliability().kind != from.reliability().kind) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Reliability Kind cannot be changed after the "
                         "creation of a DataWriter.");
  }
  if (to.ownership().kind != from.ownership().kind) {
    ...
    EPROSIMA_LOG_WARNING(
        RTPS_QOS_CHECK,
        "Ownership Kind cannot be changed after the creation of a DataWriter.");
  }
  if (to.destination_order().kind != from.destination_order().kind) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Destination order Kind cannot be changed after the "
                         "creation of a DataWriter.");
  }
  if (to.data_sharing().kind() != from.data_sharing().kind()) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Data sharing configuration cannot be changed after "
                         "the creation of a DataWriter.");
  }
  if (to.data_sharing().shm_directory() !=
      from.data_sharing().shm_directory()) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "Data sharing configuration cannot be changed after "
                         "the creation of a DataWriter.");
...
std::shared_ptr<IPayloadPool> DataWriterImpl::get_payload_pool() {
  if (!payload_pool_) {
    // When the user requested PREALLOCATED_WITH_REALLOC, but we know the type
    // cannot grow, we translate the policy into bare PREALLOCATED
    ...
    fixed_payload_size_ = config.memory_policy == PREALLOCATED_MEMORY_MODE
    ...
    if (is_data_sharing_compatible_) {
      ...
      if (!std::static_pointer_cast<ITopicPayloadPool>(payload_pool_)
               ->reserve_history(config, false)) {
        payload_pool_.reset();
      }
    }
...
bool DataWriterImpl::release_payload_pool() {
  ...
  if (is_data_sharing_compatible_) {
    ...
    PoolConfig config = PoolConfig::from_history_attributes(history_.m_att);
    auto topic_pool =
        std::static_pointer_cast<ITopicPayloadPool>(payload_pool_);
...
bool DataWriterImpl::add_loan(void* data, PayloadInfo_t& payload) {
  return loans_ && loans_->add_loan(data, payload);
}

bool DataWriterImpl::check_and_remove_loan(void* data, PayloadInfo_t& payload) {
...
#if HAVE_SECURITY
  bool has_security_enabled =
      publisher_->rtps_participant()->is_security_enabled_for_writer(
          writer_attributes);
#else
  ...
  switch (qos_.data_sharing().kind()) {
    ...
    case DataSharingKind::ON:
      ...
      if (has_security_enabled) {
        EPROSIMA_LOG_ERROR(
            DATA_WRITER,
            "Data sharing cannot be used with security protection.");
      ...
      if (!has_bound_payload_size) {
        EPROSIMA_LOG_ERROR(
            DATA_WRITER, "Data sharing cannot be used with "
                             << (type_.is_bounded()
                                     ? "memory policies other than PREALLOCATED"
      ...
      if (has_key) {
        EPROSIMA_LOG_ERROR(DATA_WRITER,
                           "Data sharing cannot be used with keyed data types");
        return ReturnCode_t::RETCODE_BAD_PARAMETER;
      }
    ...
    case DataSharingKind::AUTO:
      ...
      if (has_security_enabled) {
        EPROSIMA_LOG_INFO(
            DATA_WRITER,
            "Data sharing disabled due to security configuration.");
        return ReturnCode_t::RETCODE_OK;
      }
#endif  // HAVE_SECURITY
      ...
      if (!has_bound_payload_size) {
        EPROSIMA_LOG_INFO(
            DATA_WRITER,
            "Data sharing disabled because "
                << (type_.is_bounded() ? "memory policy is not PREALLOCATED"
      ...
      if (has_key) {
        EPROSIMA_LOG_INFO(DATA_WRITER,
                          "Data sharing disabled because data type is keyed");
        return ReturnCode_t::RETCODE_OK;
      }
...
void DataWriterImpl::process_reader_filter_info(
  ...
  if (reader_filters_ &&
    ...
    reader_filters_->process_reader_filter_info(
        reader_guid, reader_info.content_filter(),
        publisher_->get_participant_impl(), topic_);
  }
}

void DataWriterImpl::filter_is_being_removed(const char* filter_class_name) {
  if (reader_filters_) {
    assert(writer_);
...
bool DataWriterImpl::is_relevant(
  ...
  const DataWriterFilteredChange& writer_change =
      static_cast<const DataWriterFilteredChange&>(change);
