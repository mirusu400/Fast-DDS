...
PublisherImpl::PublisherImpl(DomainParticipantImpl* p, const PublisherQos& qos,
                             PublisherListener* listen)
    : participant_(p),
      qos_(&qos == &PUBLISHER_QOS_DEFAULT
               ? participant_->get_default_publisher_qos()
               : qos),
      listener_(listen),
...
const PublisherQos& PublisherImpl::get_qos() const { return qos_; }
...
ReturnCode_t PublisherImpl::set_listener(PublisherListener* listener) {
  listener_ = listener;
  return ReturnCode_t::RETCODE_OK;
}
...
void PublisherImpl::PublisherWriterListener::on_offered_deadline_missed(
    DataWriter* writer, const fastrtps::OfferedDeadlineMissedStatus& status) {
  if (publisher_->listener_ != nullptr) {
...
DataWriterImpl* PublisherImpl::create_datawriter_impl(
    const TypeSupport& type, Topic* topic, const DataWriterQos& qos,
    DataWriterListener* listener) {
  return new DataWriterImpl(this, type, topic, qos, listener);
}

DataWriter* PublisherImpl::create_datawriter(Topic* topic,
                                             const DataWriterQos& qos,
                                             DataWriterListener* listener,
                                             const StatusMask& mask) {
  EPROSIMA_LOG_INFO(PUBLISHER,
                    "CREATING WRITER IN TOPIC: " << topic->get_name());
  ...
  DataWriterImpl* impl =
      create_datawriter_impl(type_support, topic, qos, listener);
...
DataWriter* PublisherImpl::create_datawriter(Topic* topic, DataWriterImpl* impl,
                                             const StatusMask& mask) {
...
DataWriter* PublisherImpl::create_datawriter_with_profile(
    Topic* topic, const std::string& profile_name, DataWriterListener* listener,
    const StatusMask& mask) {
...
ReturnCode_t PublisherImpl::delete_datawriter(const DataWriter* writer) {
  ...
  if (vit != writers_.end()) {
    auto dw_it =
        std::find(vit->second.begin(), vit->second.end(), writer->impl_);
...
DataWriter* PublisherImpl::lookup_datawriter(
  ...
  if (it != writers_.end() && it->second.size() > 0) {
    DataWriter* writer = it->second.front()->user_datawriter_;
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
bool PublisherImpl::get_datawriters(std::vector<DataWriter*>& writers) const {
  std::lock_guard<std::mutex> lock(mtx_writers_);
...
void PublisherImpl::reset_default_datawriter_qos() {
  // TODO (ILG): Change when we have full XML support for DDS QoS profiles
  DataWriterImpl::set_qos(default_datawriter_qos_, DATAWRITER_QOS_DEFAULT,
                          true);
  PublisherAttributes attr;
...
const DataWriterQos& PublisherImpl::get_default_datawriter_qos() const {
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "PublisherImpl::get_default_datawriter_qos\t%p\n",
...
const ReturnCode_t PublisherImpl::get_datawriter_qos_from_profile(
    const std::string& profile_name, DataWriterQos& qos) const {
  PublisherAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillPublisherAttributes(profile_name, attr, false)) {
...
const DomainParticipant* PublisherImpl::get_participant() const {
  const DomainParticipant* dp =
      const_cast<const DomainParticipantImpl*>(participant_)->get_participant();
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
ReturnCode_t PublisherImpl::delete_contained_entities() {
  ...
  for (auto writer : writers_) {
    for (DataWriterImpl* dw : writer.second) {
      can_be_deleted =
          dw->check_delete_preconditions() == ReturnCode_t::RETCODE_OK;
      if (!can_be_deleted) {
        FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
bool PublisherImpl::can_be_deleted() {
  ...
  for (auto topic_writers : writers_) {
    for (DataWriterImpl* dw : topic_writers.second) {
      can_be_deleted = can_be_deleted && (dw->check_delete_preconditions() ==
                                          ReturnCode_t::RETCODE_OK);
      if (!can_be_deleted) {
...
bool PublisherImpl::type_in_use(const std::string& type_name) const {
...
void PublisherImpl::set_qos(PublisherQos& to, const PublisherQos& from,
                            bool first_time) {
...
ReturnCode_t PublisherImpl::check_qos(const PublisherQos& qos) {
  (void)qos;
  return ReturnCode_t::RETCODE_OK;
}

bool PublisherImpl::can_qos_be_updated(const PublisherQos& to,
