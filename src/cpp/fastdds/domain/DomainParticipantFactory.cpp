...
ReturnCode_t DomainParticipantFactory::delete_participant(
  ...
  using VectorIt =
      std::map<DomainId_t, std::vector<DomainParticipantImpl*>>::iterator;
  ...
  if (part != nullptr) {
    ...
    eprosima::fastdds::statistics::dds::DomainParticipantImpl* stat_part_impl =
        static_cast<eprosima::fastdds::statistics::dds::DomainParticipantImpl*>(
            part->impl_);
  ...
  if (fastrtps::rtps::GUID_t::unknown() != dom_part_impl->guid()) {
    {
      ...
      if (vector_it == participants_.end()) {
        ...
        auto pair_it =
            participants_.insert(std::make_pair(did, std::move(new_vector)));
...
DomainParticipant* DomainParticipantFactory::create_participant_with_profile(
    DomainId_t did, const std::string& profile_name,
    DomainParticipantListener* listen, const StatusMask& mask) {
  load_profiles();
...
DomainParticipant* DomainParticipantFactory::create_participant_with_profile(
    const std::string& profile_name, DomainParticipantListener* listen,
...
ReturnCode_t DomainParticipantFactory::get_participant_qos_from_profile(
    const std::string& profile_name, DomainParticipantQos& qos) const {
  ParticipantAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillParticipantAttributes(profile_name, attr, false)) {
    qos = default_participant_qos_;
    utils::set_qos_from_attributes(qos, attr.rtps);
    return ReturnCode_t::RETCODE_OK;
  }
...
ReturnCode_t DomainParticipantFactory::load_profiles() {
  // NOTE: This could be done with a bool atomic to avoid taking the mutex in
  // most cases, however the use of atomic over mutex is not deterministically
  // better, and this way is easier to read and understand.
...
ReturnCode_t DomainParticipantFactory::load_XML_profiles_file(
  ...
  if (XMLP_ret::XML_ERROR == XMLProfileManager::loadXMLFile(xml_profile_file)) {
    EPROSIMA_LOG_ERROR(DOMAIN,
                       "Problem loading XML file '" << xml_profile_file << "'");
    return ReturnCode_t::RETCODE_ERROR;
  }
  return ReturnCode_t::RETCODE_OK;
}
...
void DomainParticipantFactory::reset_default_participant_qos() {
  ...
  DomainParticipantImpl::set_qos(default_participant_qos_,
                                 PARTICIPANT_QOS_DEFAULT, true);
...
void DomainParticipantFactory::set_qos(DomainParticipantFactoryQos& to,
