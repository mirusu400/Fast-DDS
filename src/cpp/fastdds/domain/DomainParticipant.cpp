...
DomainParticipant::DomainParticipant(const StatusMask& mask)
    : Entity(mask), impl_(nullptr) {}

DomainParticipant::~DomainParticipant() {
  if (impl_) {
    DomainParticipantFactory::get_instance()->participant_has_been_deleted(
        impl_);
...
ReturnCode_t DomainParticipant::get_qos(DomainParticipantQos& qos) const {
...
ReturnCode_t DomainParticipant::set_qos(const DomainParticipantQos& qos) const {
...
ReturnCode_t DomainParticipant::set_listener(
    DomainParticipantListener* listener, const std::chrono::seconds timeout) {
  return set_listener(listener, StatusMask::all(), timeout);
}

ReturnCode_t DomainParticipant::set_listener(
    DomainParticipantListener* listener, const StatusMask& mask) {
...
Publisher* DomainParticipant::create_publisher(const PublisherQos& qos,
...
ReturnCode_t DomainParticipant::delete_publisher(const Publisher* publisher) {
  return impl_->delete_publisher(publisher);
}

Subscriber* DomainParticipant::create_subscriber(const SubscriberQos& qos,
...
Topic* DomainParticipant::create_topic(const std::string& topic_name,
...
Topic* DomainParticipant::create_topic_with_profile(
    ...
    const StatusMask& mask) {
  return impl_->create_topic_with_profile(topic_name, type_name, profile_name,
                                          listener, mask);
}
...
ContentFilteredTopic* DomainParticipant::create_contentfilteredtopic(
  ...
  return impl_->create_contentfilteredtopic(
      name, related_topic, filter_expression, expression_parameters,
...
MultiTopic* DomainParticipant::create_multitopic(
  ...
  EPROSIMA_LOG_WARNING(DOMAIN_PARTICIPANT,
                       "create_multitopic method not implemented");
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DomainParticipant::create_multitopic\t%d\n", nullptr);
  fclose(fp);
  return nullptr;
}
...
ReturnCode_t DomainParticipant::delete_multitopic(
  ...
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DomainParticipant::delete_multitopic\t%d\n",
...
ReturnCode_t DomainParticipant::register_content_filter_factory(
  ...
  return impl_->register_content_filter_factory(filter_class_name,
                                                filter_factory);
}
...
Topic* DomainParticipant::find_topic(const std::string& topic_name,
...
const Subscriber* DomainParticipant::get_builtin_subscriber() const {
  EPROSIMA_LOG_WARNING(DOMAIN_PARTICIPANT,
                       "get_builtin_subscriber method not implemented");
  return nullptr;
}
...
ReturnCode_t DomainParticipant::ignore_topic(const InstanceHandle_t& handle) {
...
ReturnCode_t DomainParticipant::get_default_topic_qos(TopicQos& qos) const {
...
bool DomainParticipant::contains_entity(const InstanceHandle_t& a_handle,
...
ReturnCode_t DomainParticipant::register_type(TypeSupport type,
                                              const std::string& type_name) {
...
ReturnCode_t DomainParticipant::unregister_type(const std::string& typeName) {
  return impl_->unregister_type(typeName);
}

TypeSupport DomainParticipant::find_type(const std::string& type_name) const {
...
bool DomainParticipant::new_remote_endpoint_discovered(
    const fastrtps::rtps::GUID_t& partguid, uint16_t userId,
