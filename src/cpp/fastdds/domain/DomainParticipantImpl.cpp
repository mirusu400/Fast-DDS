...
DomainParticipantImpl::DomainParticipantImpl(DomainParticipant* dp,
                                             DomainId_t did,
...
#pragma warning(disable : 4355)
  ...
  if (!eprosima::fastrtps::rtps::RTPSDomainImpl::create_participant_guid(
          participant_id_, guid_)) {
    EPROSIMA_LOG_ERROR(DOMAIN_PARTICIPANT,
                       "Error generating GUID for participant");
  }

  /* Fill physical data properties if they are found and empty */
  std::string* property_value =
      fastrtps::rtps::PropertyPolicyHelper::find_property(
  ...
  if (nullptr != property_value && property_value->empty()) {
    property_value->assign(asio::ip::host_name() + ":" +
                           std::to_string(utils::default_domain_id()));
  }
...
void DomainParticipantImpl::disable() {
  ...
  // DomainParticipantFactory::delete_participant() and DomainParticipantFactory
  // destructor.
  ...
  if (rtps_participant != nullptr) {
...

    {
      ...
      for (auto pub_it = publishers_.begin(); pub_it != publishers_.end();
           ++pub_it) {
...

    {
      ...
      for (auto sub_it = subscribers_.begin(); sub_it != subscribers_.end();
           ++sub_it) {
...
DomainParticipantImpl::~DomainParticipantImpl() {
...

  {
    ...
    for (auto topic_it = topics_.begin(); topic_it != topics_.end();
         ++topic_it) {
  ...
  // Note that this should never occur since reception and events threads joined
  // when removing rtps_participant.
...
ReturnCode_t DomainParticipantImpl::enable() {
  ...
  // If DEFAULT_ROS2_MASTER_URI is specified then try to create default client
  // if that already exists.
  RTPSParticipant* part =
      RTPSDomainImpl::clientServerEnvironmentCreationOverride(
          domain_id_, false, rtps_attr, &rtps_listener_);

  if (part == nullptr) {
    part = RTPSDomain::createParticipant(domain_id_, false, rtps_attr,
                                         &rtps_listener_);
...
ReturnCode_t DomainParticipantImpl::set_qos(const DomainParticipantQos& qos) {
...

  {
    ...
    const DomainParticipantQos& qos_to_set =
...
ReturnCode_t DomainParticipantImpl::delete_publisher(const Publisher* pub) {
  ...
  if (pit != publishers_.end()) {
    assert(pub->get_instance_handle() == pit->second->get_instance_handle() &&
           "The publisher instance handle does not match the publisher "
           "implementation instance handle");
    ...
    publishers_by_handle_.erase(
        publishers_by_handle_.find(pit->second->get_instance_handle()));
    delete pit->second;
...
ReturnCode_t DomainParticipantImpl::delete_subscriber(const Subscriber* sub) {
  ...
  if (sit != subscribers_.end()) {
    assert(sub->get_instance_handle() == sit->second->get_instance_handle() &&
           "The subscriber instance handle does not match the subscriber "
           "implementation instance handle");
    ...
    subscribers_by_handle_.erase(
        subscribers_by_handle_.find(sit->second->get_instance_handle()));
    delete sit->second;
...
Topic* DomainParticipantImpl::find_topic(const std::string& topic_name,
                                         const fastrtps::Duration_t& timeout) {
  auto find_fn = [this, &topic_name]() {
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "DomainParticipantImpl::find_topic\t%d\n",
            topics_.count(topic_name) > 0);
    fclose(fp);
  ...
  if (timeout.is_infinite()) {
    ...
    auto duration = std::chrono::seconds(timeout.seconds) +
...
void DomainParticipantImpl::set_topic_listener(const TopicProxyFactory* factory,
                                               TopicImpl* impl,
...
ReturnCode_t DomainParticipantImpl::delete_topic(const Topic* topic) {
  if (topic == nullptr) {
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  ...
  auto handle_it = std::find_if(
      topics_by_handle_.begin(), topics_by_handle_.end(),
...
ContentFilteredTopic* DomainParticipantImpl::create_contentfilteredtopic(
  ...
  if (related_topic->get_participant() != get_participant()) {
    EPROSIMA_LOG_ERROR(PARTICIPANT,
                       "Creating ContentFilteredTopic with name "
                           << name
  ...
  IContentFilterFactory* filter_factory =
      find_content_filter_factory(filter_class_name);
  if (nullptr == filter_factory) {
    EPROSIMA_LOG_ERROR(PARTICIPANT, "Could not find factory for filter class "
                                        << filter_class_name);
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "DomainParticipantImpl::create_contentfilteredtopic\t%d\n",
            nullptr);
  ...
  if (expression_parameters.size() > 100) {
    EPROSIMA_LOG_ERROR(
        PARTICIPANT,
        "Number of expression parameters exceeds maximum protocol limit: "
  ...
  n_params = static_cast<LoanableSequence<const char*>::size_type>(
      expression_parameters.size());
  ...
  if (ReturnCode_t::RETCODE_OK !=
      filter_factory->create_content_filter(
          filter_class_name, related_topic->get_type_name().c_str(), type.get(),
          filter_expression.c_str(), filter_parameters, filter_instance)) {
    EPROSIMA_LOG_ERROR(PARTICIPANT, "Could not create filter of class "
                                        << filter_class_name
                                        << " for expression \""
                                        << filter_expression);
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  ...
  topic = new ContentFilteredTopic(name, related_topic, filter_expression,
                                   expression_parameters);
  ContentFilteredTopicImpl* content_topic_impl =
      static_cast<ContentFilteredTopicImpl*>(topic->get_impl());
  ...
  filtered_topics_.emplace(
      std::make_pair(name, std::unique_ptr<ContentFilteredTopic>(topic)));
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "DomainParticipantImpl::create_contentfilteredtopic\t%p\n",
          topic);
  fclose(fp);
  return topic;
}
...
ReturnCode_t DomainParticipantImpl::register_content_filter_factory(
  ...
  if (nullptr == filter_factory || nullptr == filter_class_name ||
      strlen(filter_class_name) > 255) {
    return ReturnCode_t::RETCODE_BAD_PARAMETER;
  }
  ...
  if ((it != filter_factories_.end()) ||
      (0 == strcmp(filter_class_name, FASTDDS_SQLFILTER_NAME))) {
    return ReturnCode_t::RETCODE_PRECONDITION_NOT_MET;
  }
...
IContentFilterFactory* DomainParticipantImpl::lookup_content_filter_factory(
  ...
  auto it = filter_factories_.find(filter_class_name);
  if ((it == filter_factories_.end()) ||
      (it->first == FASTDDS_SQLFILTER_NAME)) {
    return nullptr;
  }
  return it->second;
}
...
ReturnCode_t DomainParticipantImpl::unregister_content_filter_factory(
  ...
  for (auto& topic : filtered_topics_) {
    if (topic.second->impl_->filter_property.filter_class_name ==
        filter_class_name) {
      return ReturnCode_t::RETCODE_PRECONDITION_NOT_MET;
    }
  }
...
const GUID_t& DomainParticipantImpl::guid() const { return guid_; }

Publisher* DomainParticipantImpl::create_publisher(const PublisherQos& qos,
...
Publisher* DomainParticipantImpl::create_publisher(const PublisherQos& qos,
  ...
  if (!PublisherImpl::check_qos(qos)) {
    // The PublisherImpl::check_qos() function is not yet implemented and always
    // returns ReturnCode_t::RETCODE_OK. It will be implemented in future
    // releases of Fast DDS. EPROSIMA_LOG_ERROR(PARTICIPANT, "PublisherQos
    // inconsistent or not supported"); return nullptr;
  }
...
Publisher* DomainParticipantImpl::create_publisher_with_profile(
    const std::string& profile_name, PublisherListener* listener,
    const StatusMask& mask) {
  // TODO (ILG): Change when we have full XML support for DDS QoS profiles
  PublisherAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillPublisherAttributes(profile_name, attr)) {
...
PublisherImpl* DomainParticipantImpl::create_publisher_impl(
    const PublisherQos& qos, PublisherListener* listener) {
...
ReturnCode_t DomainParticipantImpl::ignore_participant(
  ...
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(
      fp, "DomainParticipantImpl::ignore_participant()\t%d\n",
      (nullptr == rtps_participant_) ? ReturnCode_t::RETCODE_NOT_ENABLED
      : rtps_participant_->ignore_participant(iHandle2GUID(handle).guidPrefix)
          ? ReturnCode_t::RETCODE_OK
          : ReturnCode_t::RETCODE_BAD_PARAMETER);
  fclose(fp);
  return (nullptr == rtps_participant_) ? ReturnCode_t::RETCODE_NOT_ENABLED
         : rtps_participant_->ignore_participant(
               iHandle2GUID(handle).guidPrefix)
             ? ReturnCode_t::RETCODE_OK
             : ReturnCode_t::RETCODE_BAD_PARAMETER;
}
...
bool DomainParticipantImpl::ignore_publication(const InstanceHandle_t& handle) {
...
ReturnCode_t DomainParticipantImpl::delete_contained_entities() {
  ...
  while (it_subs != subscribers_.end()) {
    ...
    subscribers_by_handle_.erase(
        it_subs->second->get_subscriber()->get_instance_handle());
  ...
  while (it_pubs != publishers_.end()) {
    ...
    publishers_by_handle_.erase(
        it_pubs->second->get_publisher()->get_instance_handle());
...
ReturnCode_t DomainParticipantImpl::set_default_publisher_qos(
  ...
  if (!ret_val) {
    // The PublisherImpl::check_qos() function is not yet implemented and always
    // returns ReturnCode_t::RETCODE_OK. It will be implemented in future
    // releases of Fast DDS. return ret_val;
  }
...
const ReturnCode_t DomainParticipantImpl::get_publisher_qos_from_profile(
    const std::string& profile_name, PublisherQos& qos) const {
  PublisherAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillPublisherAttributes(profile_name, attr)) {
...
ReturnCode_t DomainParticipantImpl::set_default_subscriber_qos(
  ...
  if (!check_result) {
    // The SubscriberImpl::check_qos() function is not yet implemented and
    // always returns ReturnCode_t::RETCODE_OK. It will be implemented in future
    // releases of Fast DDS. return check_result;
  }
...
const ReturnCode_t DomainParticipantImpl::get_subscriber_qos_from_profile(
    const std::string& profile_name, SubscriberQos& qos) const {
  SubscriberAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillSubscriberAttributes(profile_name, attr)) {
...
const ReturnCode_t DomainParticipantImpl::get_topic_qos_from_profile(
    const std::string& profile_name, TopicQos& qos) const {
  TopicAttributes attr;
  if (XMLP_ret::XML_OK ==
      XMLProfileManager::fillTopicAttributes(profile_name, attr)) {
...
bool DomainParticipantImpl::contains_entity(const InstanceHandle_t& handle,
...
Subscriber* DomainParticipantImpl::create_subscriber(
    const SubscriberQos& qos, SubscriberListener* listener,
    const StatusMask& mask) {
  if (!SubscriberImpl::check_qos(qos)) {
    // The SubscriberImpl::check_qos() function is not yet implemented and
    // always returns ReturnCode_t::RETCODE_OK. It will be implemented in future
    // releases of Fast DDS. EPROSIMA_LOG_ERROR(PARTICIPANT, "SubscriberQos
    // inconsistent or not supported"); return nullptr;
  }
...
Subscriber* DomainParticipantImpl::create_subscriber_with_profile(
    const std::string& profile_name, SubscriberListener* listener,
    const StatusMask& mask) {
  // TODO (ILG): Change when we have full XML support for DDS QoS profiles
  SubscriberAttributes attr;
  if (XMLP_ret::XML_OK ==
...
Topic* DomainParticipantImpl::create_topic(const std::string& topic_name,
...
ReturnCode_t DomainParticipantImpl::register_type(
    const TypeSupport type, const std::string& type_name) {
  if (type_name.size() <= 0) {
  ...
  if (!t.empty()) {
    ...
    EPROSIMA_LOG_ERROR(PARTICIPANT, "Another type with the same name '"
                                        << type_name
...
bool DomainParticipantImpl::register_dynamic_type_to_factories(
  ...
  if (dpst != nullptr)  // Registering a dynamic type.
  {
    ...
    DynamicTypeBuilderFactory* dynFactory =
        DynamicTypeBuilderFactory::get_instance();
    const TypeIdentifier* id =
        objectFactory->get_type_identifier_trying_complete(dpst->getName());
    if (id == nullptr) {
      ...
      TypeObject typeObj;
      dynFactory->build_type_object(
          dpst->GetDynamicType()->get_type_descriptor(), typeObj, &members);
      // Minimal too
      dynFactory->build_type_object(
          dpst->GetDynamicType()->get_type_descriptor(), typeObj, &members,
          false);
      const TypeIdentifier* type_id2 =
          objectFactory->get_type_identifier(dpst->getName());
      const TypeObject* type_obj =
          objectFactory->get_type_object(dpst->getName());
      if (type_id2 == nullptr) {
        EPROSIMA_LOG_ERROR(DOMAIN_PARTICIPANT,
        ...
        const TypeIdentifier* type_id_complete =
            objectFactory->get_type_identifier(dpst->getName(), true);
        const TypeObject* type_obj_complete =
            objectFactory->get_type_object(dpst->getName(), true);
        objectFactory->add_type_object(dpst->getName(), type_id_complete,
...
ReturnCode_t DomainParticipantImpl::unregister_type(
  ...
  if (t.empty()) {
    return ReturnCode_t::RETCODE_OK;  // Not registered, so unregistering
                                      // complete.
  }
...
void DomainParticipantImpl::MyRTPSParticipantListener::onParticipantDiscovery(
    RTPSParticipant*, ParticipantDiscoveryInfo&& info,
    bool& should_be_ignored) {
  should_be_ignored = false;
  Sentry sentinel(this);
  if (sentinel) {
    participant_->listener_->on_participant_discovery(
        participant_->participant_, std::move(info), should_be_ignored);
    if (!should_be_ignored) {
      participant_->listener_->on_participant_discovery(
          participant_->participant_, std::move(info));
    }
  }
}

#if HAVE_SECURITY
void DomainParticipantImpl::MyRTPSParticipantListener::
    onParticipantAuthentication(RTPSParticipant*,
                                ParticipantAuthenticationInfo&& info) {
  Sentry sentinel(this);
  if (sentinel) {
    participant_->listener_->onParticipantAuthentication(
        participant_->participant_, std::move(info));
  }
}
...
void DomainParticipantImpl::MyRTPSParticipantListener::onReaderDiscovery(
    RTPSParticipant*, ReaderDiscoveryInfo&& info) {
  Sentry sentinel(this);
  if (sentinel) {
    participant_->listener_->on_subscriber_discovery(participant_->participant_,
                                                     std::move(info));
  }
}

void DomainParticipantImpl::MyRTPSParticipantListener::onWriterDiscovery(
    RTPSParticipant*, WriterDiscoveryInfo&& info) {
  Sentry sentinel(this);
  if (sentinel) {
    participant_->listener_->on_publisher_discovery(participant_->participant_,
                                                    std::move(info));
  }
}

void DomainParticipantImpl::MyRTPSParticipantListener::on_type_discovery(
    RTPSParticipant*, const fastrtps::rtps::SampleIdentity& request_sample_id,
  ...
  Sentry sentinel(this);
  if (sentinel) {
    participant_->listener_->on_type_discovery(participant_->participant_,
                                               request_sample_id, topic,
                                               identifier, object, dyn_type);

    participant_->check_get_type_request(request_sample_id, identifier, object,
                                         dyn_type);
  }
}
...
void DomainParticipantImpl::MyRTPSParticipantListener::
  ...
  if (sentinel) {
    ...
    participant_->check_get_dependencies_request(request_sample_id,
                                                 dependencies);
  }
}
...
void DomainParticipantImpl::MyRTPSParticipantListener::
    on_type_information_received(
        RTPSParticipant*, const fastrtps::string_255& topic_name,
...
bool DomainParticipantImpl::new_remote_endpoint_discovered(
    const GUID_t& partguid, uint16_t endpointId, EndpointKind_t kind) {
  if (get_rtps_participant() != nullptr) {
    if (kind == fastrtps::rtps::WRITER) {
      return get_rtps_participant()->newRemoteWriterDiscovered(
          partguid, static_cast<int16_t>(endpointId));
    } else {
      return get_rtps_participant()->newRemoteReaderDiscovered(
          partguid, static_cast<int16_t>(endpointId));
    }
  }
...
fastrtps::rtps::SampleIdentity DomainParticipantImpl::get_type_dependencies(
  ...
  const fastrtps::rtps::RTPSParticipant* rtps_participant =
...
ReturnCode_t DomainParticipantImpl::register_remote_type(
  ...
  // Check if plain
  if (type_information.complete().typeid_with_size().type_id()._d() <
      EK_MINIMAL) {
    DynamicType_ptr dyn = factory->build_dynamic_type(
        type_name, &type_information.minimal().typeid_with_size().type_id());

    if (nullptr != dyn) {
      // callback(type_name, dyn); // For plain types, don't call the callback
      return register_dynamic_type(dyn);
    }
    // If cannot create the dynamic type, probably is because it depend on
    // unknown types. We must continue.
  }

  // Check if already available
  TypeObject obj;
  factory->typelookup_get_type(
      type_information.complete().typeid_with_size().type_id(), obj);

  if (obj._d() != 0) {
    DynamicType_ptr dyn = factory->build_dynamic_type(
        type_name, &type_information.complete().typeid_with_size().type_id(),
        &obj);

    if (nullptr != dyn) {
      // callback(type_name, dyn); // If the type is already registered, don't
      // call the callback.
      return register_dynamic_type(dyn);
    }
    ...
    fill_pending_dependencies(type_information.complete().dependent_typeids(),
                              dependencies, retrieve_objects);

    fastrtps::rtps::SampleIdentity request_dependencies;
    fastrtps::rtps::SampleIdentity request_objects;

    // Lock now, we don't want to process the reply before we add the requests'
    // ID to the maps.
    ...
    // If no more dependencies but failed to create, probably we only need the
    // TypeObject
    ...
    register_callbacks_.emplace(
        std::make_pair(requestId, std::make_pair(type_name, callback)));
...
bool DomainParticipantImpl::check_get_type_request(
  ...
  if (builtin::INVALID_SAMPLE_IDENTITY != requestId) {
    ...
    if (cb_it != register_callbacks_.end()) {
      ...
      // Register the received TypeObject into factory and recreate the
      // DynamicType.
      ...
      if (pending != parent_requests_.end() &&
          pending->second.size() < 2)  // Exists and everything is solved.
      {
        fastrtps::types::DynamicType_ptr dynamic =
            fastrtps::types::TypeObjectFactory::get_instance()
                ->build_dynamic_type(name, identifier, object);
      ...
      // Failed, cannot register the type yet, probably child request still
      // pending.
      return false;
    }
    ...
    if (child_it != child_requests_.end()) {
      // Register received TypeObject into factory, remove the iterator from the
      // map and check our parent.
      fastrtps::types::TypeObjectFactory::get_instance()->add_type_object(
...
void DomainParticipantImpl::fill_pending_dependencies(
  ...
  for (const TypeIdentifierWithSize& tiws : dependencies) {
    ...
    if (!TypeObjectFactory::get_instance()->typelookup_check_type_identifier(
            tiws.type_id())) {
    ...
    if (tiws.type_id()._d() >= EK_MINIMAL) {
      TypeObject obj;
      TypeObjectFactory::get_instance()->typelookup_get_type(tiws.type_id(),
                                                             obj);
      if (obj._d() == 0) {
...
bool DomainParticipantImpl::check_get_dependencies_request(
  ...
  if (builtin::INVALID_SAMPLE_IDENTITY != requestId) {
    ...
    if (cb_it != register_callbacks_.end()) {
      fill_pending_dependencies(dependencies, next_dependencies,
                                retrieve_objects);

      // If any pending dependency exists, retrieve it
      if (!next_dependencies.empty()) {
        fastrtps::rtps::SampleIdentity child_request =
            get_type_dependencies(next_dependencies);
        std::vector<fastrtps::rtps::SampleIdentity> vector;
...
void DomainParticipantImpl::on_child_requests_finished(
  ...
  if (parent_requests_.end() == pending_requests_it ||
      pending_requests_it->second.empty()) {
    // Am I a children?
    ...
    if (child_requests_.end() != child_it) {
      ...
      cb_it->second.second(
          cb_it->second.first,
          fastrtps::types::DynamicType_ptr(
              nullptr));  // Everything should be already registered
      register_callbacks_.erase(cb_it);
    }
  }
}
...
bool DomainParticipantImpl::set_qos(DomainParticipantQos& to,
...
fastrtps::types::ReturnCode_t DomainParticipantImpl::check_qos(
  ...
  if (qos.allocation().data_limits.max_user_data == 0 ||
      qos.allocation().data_limits.max_user_data >
...
bool DomainParticipantImpl::can_qos_be_updated(
    const DomainParticipantQos& to, const DomainParticipantQos& from) {
  bool updatable = true;
  if (!(to.allocation() == from.allocation())) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "ParticipantResourceLimitsQos cannot be changed after "
  ...
  if (!(to.properties() == from.properties())) {
    ...
    EPROSIMA_LOG_WARNING(
        RTPS_QOS_CHECK,
        "PropertyPolilyQos cannot be changed after the participant is enabled");
  }
  if (!(to.wire_protocol() == from.wire_protocol())) {
    // Check that the only modification was in
    // wire_protocol().discovery_config.m_DiscoveryServers
    if ((to.wire_protocol().builtin.discovery_config.m_DiscoveryServers ==
        ...
        (!(to.wire_protocol().builtin.discovery_config.m_DiscoveryServers ==
         ...
         (!(to.wire_protocol().prefix == from.wire_protocol().prefix) ||
          !(to.wire_protocol().participant_id ==
            from.wire_protocol().participant_id) ||
          !(to.wire_protocol().port == from.wire_protocol().port) ||
          !(to.wire_protocol().throughput_controller ==
            from.wire_protocol().throughput_controller) ||
          ...
          !(to.wire_protocol().builtin.initialPeersList ==
            from.wire_protocol().builtin.initialPeersList) ||
          !(to.wire_protocol().builtin.readerHistoryMemoryPolicy ==
            from.wire_protocol().builtin.readerHistoryMemoryPolicy) ||
          !(to.wire_protocol().builtin.readerPayloadSize ==
          ...
          !(to.wire_protocol().builtin.writerPayloadSize ==
            from.wire_protocol().builtin.writerPayloadSize) ||
          !(to.wire_protocol().builtin.mutation_tries ==
            from.wire_protocol().builtin.mutation_tries) ||
          ...
          !(to.wire_protocol()
            ...
            from.wire_protocol()
                ...
                .use_STATIC_EndpointDiscoveryProtocol) ||
          !(to.wire_protocol()
                .builtin.discovery_config.discoveryServer_client_syncperiod ==
            from.wire_protocol()
                .builtin.discovery_config.discoveryServer_client_syncperiod) ||
          ...
          !(to.wire_protocol().builtin.discovery_config.initial_announcements ==
            from.wire_protocol()
                .builtin.discovery_config.initial_announcements) ||
          !(to.wire_protocol().builtin.discovery_config.m_simpleEDP ==
            from.wire_protocol().builtin.discovery_config.m_simpleEDP) ||
          !(strcmp(to.wire_protocol()
                       .builtin.discovery_config.static_edp_xml_config(),
                   from.wire_protocol()
                       .builtin.discovery_config.static_edp_xml_config()) ==
            0) ||
          !(to.wire_protocol()
                .builtin.discovery_config.ignoreParticipantFlags ==
            from.wire_protocol()
                .builtin.discovery_config.ignoreParticipantFlags)))) {
      updatable = false;
      EPROSIMA_LOG_WARNING(
          RTPS_QOS_CHECK,
          "WireProtocolConfigQos cannot be changed after the participant is "
          "enabled, "
              << "with the exception of "
                 "builtin.discovery_config.m_DiscoveryServers");
    } else {
      // This means that the only change is in
      // wire_protocol().builtin.discovery_config.m_DiscoveryServers In that
      // case, we need to ensure that the current list (to) is strictly
      // contained in the incoming list (from). For that, we check that every
      // server in the current list (to) is also in the incoming one (from)
      for (auto existing_server :
           to.wire_protocol().builtin.discovery_config.m_DiscoveryServers) {
        bool contained = false;
        for (auto incoming_server :
             from.wire_protocol().builtin.discovery_config.m_DiscoveryServers) {
        ...
        if (!contained) {
          updatable = false;
          EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                               "Discovery Servers cannot be removed from the "
                               "list; they can only be added");
          break;
        }
      }
    }
  ...
  if (!(to.transport() == from.transport())) {
    updatable = false;
    EPROSIMA_LOG_WARNING(RTPS_QOS_CHECK,
                         "TransportConfigQos cannot be changed after the "
  ...
  if (!(to.name() == from.name())) {
    ...
    EPROSIMA_LOG_WARNING(
        RTPS_QOS_CHECK,
        "Participant name cannot be changed after the participant is enabled");
  }
  return updatable;
}
