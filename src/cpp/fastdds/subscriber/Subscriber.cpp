...
Subscriber::Subscriber(SubscriberImpl* pimpl, const StatusMask& mask)
    : DomainEntity(mask), impl_(pimpl) {}

Subscriber::Subscriber(DomainParticipant* dp, const SubscriberQos& qos,
                       SubscriberListener* listener, const StatusMask& mask)
    : DomainEntity(mask),
...
const SubscriberQos& Subscriber::get_qos() const { return impl_->get_qos(); }

ReturnCode_t Subscriber::get_qos(SubscriberQos& qos) const {
  qos = impl_->get_qos();
  return ReturnCode_t::RETCODE_OK;
}
...
DataReader* Subscriber::create_datareader(TopicDescription* topic,
                                          const DataReaderQos& reader_qos,
                                          DataReaderListener* listener,
                                          const StatusMask& mask) {
  DataReader* reader =
      impl_->create_datareader(topic, reader_qos, listener, mask);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "SubscriberImpl::create_datareader\t%p\n", reader);
  fclose(fp);
  return reader;
}

DataReader* Subscriber::create_datareader_with_profile(
    TopicDescription* topic, const std::string& profile_name,
    DataReaderListener* listener, const StatusMask& mask) {
  return impl_->create_datareader_with_profile(topic, profile_name, listener,
                                               mask);
}

ReturnCode_t Subscriber::delete_datareader(const DataReader* reader) {
  ReturnCode_t ret_val = impl_->delete_datareader(reader);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
DataReader* Subscriber::lookup_datareader(const std::string& topic_name) const {
  DataReader* reader = impl_->lookup_datareader(topic_name);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
ReturnCode_t Subscriber::get_datareaders(
  ...
  ReturnCode_t ret_val = impl_->get_datareaders(readers);
  return ret_val;
}
...
bool Subscriber::has_datareaders() const { return impl_->has_datareaders(); }
...
ReturnCode_t Subscriber::notify_datareaders() const {
  ReturnCode_t ret_val = impl_->notify_datareaders();
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
ReturnCode_t Subscriber::set_default_datareader_qos(const DataReaderQos& qos) {
  ReturnCode_t ret_val = impl_->set_default_datareader_qos(qos);
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
const DataReaderQos& Subscriber::get_default_datareader_qos() const {
  const DataReaderQos& qos = impl_->get_default_datareader_qos();
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
  fprintf(fp, "SubscriberImpl::get_default_datareader_qos\t%p\n", &qos);
  fclose(fp);
  return qos;
}
...
DataReaderQos& Subscriber::get_default_datareader_qos() {
  ...
  fprintf(fp, "SubscriberImpl::get_default_datareader_qos\t%p\n", &qos);
  fclose(fp);
  return const_cast<DataReaderQos&>(qos);
}

ReturnCode_t Subscriber::get_default_datareader_qos(DataReaderQos& qos) const {
...
ReturnCode_t Subscriber::get_datareader_qos_from_profile(
    const std::string& profile_name, DataReaderQos& qos) const {
  return impl_->get_datareader_qos_from_profile(profile_name, qos);
}

ReturnCode_t Subscriber::copy_from_topic_qos(DataReaderQos& reader_qos,
...
const DomainParticipant* Subscriber::get_participant() const {
  const DomainParticipant* participant = impl_->get_participant();
  FILE* fp = fopen("/tmp/fastdds-debug", "a+");
...
const fastrtps::rtps::InstanceHandle_t& Subscriber::get_instance_handle()
    const {
