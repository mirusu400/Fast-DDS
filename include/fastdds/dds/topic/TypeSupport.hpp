 ...
 * @brief Class TypeSupport used to provide the DomainRTPSParticipant with the
 * methods to serialize, deserialize and get the key of a specific data type.
...
class TypeSupport : public std::shared_ptr<fastdds::dds::TopicDataType> {
 public:
   ...
   * @param type Another instance of TypeSupport
   */
  RTPS_DllAPI TypeSupport& operator=(const TypeSupport& type) noexcept =
      default;
   ...
   * The passed pointer will be managed by the TypeSupport object, so creating
   * two TypeSupport from the same pointer or deleting the passed pointer will
   * produce a runtime error.
   *
   * @param ptr
   */
  RTPS_DllAPI explicit TypeSupport(fastdds::dds::TopicDataType* ptr)
      : std::shared_ptr<fastdds::dds::TopicDataType>(ptr) {}

  /*!
   * @brief TypeSupport constructor that receives a DynamicPubSubType.
   * It will copy the instance so the user will keep the ownership of his
   * object.
   *
   * @param ptr
   */
  RTPS_DllAPI TypeSupport(fastrtps::types::DynamicPubSubType ptr)
      : std::shared_ptr<fastdds::dds::TopicDataType>(
            std::make_shared<fastrtps::types::DynamicPubSubType>(
                std::move(ptr))) {}

  /**
   * @brief Registers the type on a participant
   *
   * @param participant DomainParticipant where the type is going to be
   * registered
   * @return RETCODE_BAD_PARAMETER if the type name is empty,
   * RETCODE_PRECONDITION_NOT_MET if there is another type with the same name
   * registered on the DomainParticipant and RETCODE_OK if it is registered
   * correctly
   */
   ...
   * correctly
   */
  RTPS_DllAPI virtual ReturnCode_t register_type(DomainParticipant* participant,
  ...
  RTPS_DllAPI virtual const std::string& get_type_name() const {
    std::string name = get()->m_topicDataTypeName;
  ...
  RTPS_DllAPI virtual bool serialize(
      void* data, fastrtps::rtps::SerializedPayload_t* payload);
  ...
  RTPS_DllAPI virtual bool deserialize(
      fastrtps::rtps::SerializedPayload_t* payload, void* data);
   ...
   */
  RTPS_DllAPI virtual void* create_data() { return get()->createData(); }
   ...
   */
  RTPS_DllAPI virtual void delete_data(void* data) {
   ...
   */
  RTPS_DllAPI virtual bool get_key(void* data, InstanceHandle_t* i_handle,
                                   bool force_md5 = false) {
    return get()->getKey(data, i_handle, force_md5);
  }

  RTPS_DllAPI virtual bool operator==(const TypeSupport& type_support) {
   ...
   */
  RTPS_DllAPI bool empty() const { return get() == nullptr; }
   ...
   */
  RTPS_DllAPI virtual inline bool is_plain() const { return get()->is_plain(); }

  RTPS_DllAPI bool operator!=(std::nullptr_t) const { return bool(*this); }

  RTPS_DllAPI bool operator==(std::nullptr_t) const { return !*this; }
};
