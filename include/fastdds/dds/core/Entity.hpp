...
/**
 * @brief The Entity class is the abstract base class for all the objects that
...
class Entity {
 public:
   ...
   */
  RTPS_DllAPI Entity(const StatusMask& mask = StatusMask::all())
   ...
   */
  RTPS_DllAPI const StatusMask& get_status_mask() const { return status_mask_; }
   ...
   *
   * @return const reference to the StatusMask with the triggered statuses set
   * to 1
   */
  ...
  RTPS_DllAPI StatusCondition& get_statuscondition() {
    FILE* fp = fopen("/tmp/fastdds-debug", "a+");
    fprintf(fp, "Entity::get_statuscondition\t%p\n", &status_condition_);
    fclose(fp);
 ...
 protected:
   ...
   */
  RTPS_DllAPI void set_instance_handle(const InstanceHandle_t& handle) {
...
/**
 * @brief The DomainEntity class is a subclass of Entity created in order to
 * differentiate between DomainParticipants and the rest of Entities
 */
class DomainEntity : public Entity {
 public:
   ...
   * @param mask StatusMask (default: all)
   */
  DomainEntity(const StatusMask& mask = StatusMask::all()) : Entity(mask) {}
};
